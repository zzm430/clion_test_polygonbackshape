...
bool FemPosDeviationSqpOsqpInterface::Solve() {
  ...
  CalculateAffineConstraint(ref_points_,
                            &A_data,
                            &A_indices,
                            &A_indptr,
                            &lower_bounds,
  ...
  while (pen_itr < sqp_pen_max_iter_) {
    ...
    while (sub_itr < sqp_sub_max_iter_) {
      ...
      CalculateOffset(&q);
      CalculateAffineConstraint(opt_xy_,
                                &A_data,
                                &A_indices,
                                &A_indptr,
                                &lower_bounds,
      ...
      if (ftol < sqp_ftol_) {
        ...
        //        sub_itr; ADEBUG << "merit function value converges to " <<
        //        cur_fvalue
...
void FemPosDeviationSqpOsqpInterface::CalculateKernel(
  ...
  for (int col = 0; col < 2; ++col) {
    columns[col].emplace_back(col,
                              weight_fem_pos_deviation_ + weight_path_length_ +
  ...
  for (int col = second_point_col_from_last_col;
       col < last_point_col_from_last_col;
       ++col) {
