...
#include "common/math/polygon_2d.h"

namespace math{
namespace {

/**
 * @brief the distance of point (query_x, query_y) to linesegment [(start_x,
 * start_y), (end_x, end_y)], which lengh is defined by "length"
 *
 * @param query_x point x
 * @param query_y point y
 * @param start_x the x of linesegment start point
 * @param start_y the y of linesegment start point
 * @param end_x the x of linesegment end point
 * @param end_y the y of linesegment end point
 * @param length the length of linesegement
...
//        CHECK_GT(width_, -kMathEpsilon);
    ...
    bool Box2d::IsPointIn(const Vec2d &point) const {
        ...
        return dx <= half_length_ + 1e-6 && dy <= half_width_ + 1e-6;
    }
    ...
    bool Box2d::IsPointOnBoundary(const Vec2d &point) const {
        ...
        const double dx = std::abs(x0 * cos_heading_ + y0 * sin_heading_);
        const double dy = std::abs(x0 * sin_heading_ - y0 * cos_heading_);
        return (std::abs(dx - half_length_) <= 1e-6 &&
                dy <= half_width_ + 1e-6) ||
               (std::abs(dy - half_width_) <= 1e-6 &&
                dx <= half_length_ + 1e-6);
    }
    ...
    bool Box2d::HasOverlap(const LineSegment2d &line_segment) const {
        ...
        return DistanceTo(line_segment) <= 1e-6;
    }
