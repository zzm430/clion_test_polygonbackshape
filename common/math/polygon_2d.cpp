...
namespace  math{
    ...
    double Polygon2d::DistanceTo(const LineSegment2d &line_segment) const {
        if (line_segment.length() <= kMathEpsilons) {
        ...
        if (std::any_of(line_segments_.begin(),
                        line_segments_.end(),
    ...
    bool Polygon2d::HasOverlap(const Polygon2d &polygon) const {
        ...
        return DistanceTo(polygon) <= kMathEpsilons;
    }
    ...
    bool Polygon2d::Contains(const LineSegment2d &line_segment) const {
        ...
        if (!is_convex_) {
            ...
            return total_length >= line_segment.length() - kMathEpsilons;
        }
        return true;
    }
    ...
    bool Polygon2d::Contains(const Polygon2d &polygon) const {
        CHECK_GE(points_.size(), 3);
        if (area_ < polygon.area() - kMathEpsilons) {
            return false;
        }
        if (!IsPointIn(polygon.points()[0])) {
            return false;
        ...
        return std::all_of(line_segments.begin(),
                           line_segments.end(),
    ...
    bool Polygon2d::ClipConvexHull(const LineSegment2d &line_segment,
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (std::abs(prod[i]) <= kMathEpsilons) {
                side[i] = 0;
            } else {
                side[i] = ((prod[i] < 0) ? -1 : 1);
            }
    ...
    bool Polygon2d::GetOverlap(const LineSegment2d &line_segment,
        ...
        return min_proj <= max_proj + kMathEpsilons;
    }
    ...
    std::vector<LineSegment2d> Polygon2d::GetAllOverlaps(
        ...
        for (size_t i = 0; i + 1 < projections.size(); ++i) {
            ...
            if (end_proj - start_proj <= kMathEpsilons) {
                continue;
            }
            ...
            if (overlaps.empty() ||
                start_proj > overlaps.back().second + kMathEpsilons) {
    ...
    void Polygon2d::ExtremePoints(const double heading,
                                  Vec2d *const first,
                                  Vec2d *const last) const {
        CHECK_GE(points_.size(), 3);
        CHECK_NOTNULL(first);
